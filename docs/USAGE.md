...
# 2. 启动 Web 仪表盘（实时监控）
python3.12 -m uvicorn web.app:app --reload
...
```bash
python3.12 runner.py --goal "Build a Todo App" --dry-run
```
...
```bash
python3.12 runner.py --goal "Build a REST API" --ai-command "claude --print" --verbose
```
...
# 自定义端口
python3.12 -m uvicorn web.app:app --host 0.0.0.0 --port 3000
```
...
```bash
# 指定技能，跳过自动选择
python3.12 runner.py --goal "Design a landing page" --skills "ui-ux-pro-max,design-system"

# 从中断处恢复执行
python3.12 runner.py --resume --ai-command "claude --print"

# 限制迭代次数，使用退避策略
python3.12 runner.py --goal "Fix login bug" --max-iterations 10 --retry-strategy backoff

# Ralph 模式：规划后导出 PRD
python3.12 runner.py --goal "Build user auth" --execution-mode ralph

# 只生成提示词，不执行
python3.12 runner.py --goal "Build a CLI tool" --generate-prompt > prompt.txt
```
...
python3.12 runner.py --goal "Build a REST API with tests"
```
...
```bash
# 强制使用特定技能组合
python3.12 runner.py --goal "Design a dashboard" --skills "ui-ux-pro-max,design-system,brand"
```
...
```bash
python3.12 runner.py --goal "Create a new skill for database migration" --skills "write-a-skill"
```
...
```bash
# 从创意到执行的完整流程
python3.12 runner.py --goal "Build a SaaS app" --skills "grill-me,prd,to-issues,ralph"

# 设计驱动开发
python3.12 runner.py --goal "Redesign checkout flow" --skills "ui-ux-pro-max,ui-styling,design-system"

# 质量强化
python3.12 runner.py --goal "Fix production bugs" --skills "diagnose,tdd,relentless-iteration"
```
...
```bash
# 第一步：质问想法，确保方向正确
python3.12 runner.py --goal "Build a full-stack Todo app with React and FastAPI" \
